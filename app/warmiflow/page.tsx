  ...
  const getEmbedUrl = (url: string) => {
    ...
    try {
      if (url.includes("youtube.com/watch")) {
        ...
        return `https://www.youtube.com/embed/${videoId}?modestbranding=1&rel=0&fs=0`;
      }
      ...
      if (url.includes("youtube.com/embed")) {
        ...
        return `${baseUrl}?modestbranding=1&rel=0&fs=0`;
      }
  ...
  return (
    <main className="min-h-screen w-full bg-black relative font-['Inter'] flex flex-col overflow-x-hidden text-white">
      ...
      <div className="flex-1 relative z-10 w-full max-w-5xl mx-auto px-4 md:px-8 pb-20">
                ...
                {video.embedUrl ? (
                  video.embedUrl.includes("vimeo.com") || video.embedUrl.includes("youtube.com") || video.embedUrl.includes("youtu.be") || video.embedUrl.includes("drive.google.com") ? (
                    <div className="relative w-full h-full">
                      ...
                      {/* Protective Overlays for YouTube to block title/logo clicks */}
                      {(video.embedUrl.includes("youtube.com") || video.embedUrl.includes("youtu.be")) && (
                        <>
                          {/* Top block (hides title and share button) */}
        ...
        {/* ═══ Action Buttons ═══ */}
        <div className="flex flex-wrap justify-center gap-4 md:gap-6 mb-12 animate-entrance-up delay-500">
          <a
          ...
          <a
            href="https://discord.gg/5NM6tgSEM"
            target="_blank"
            rel="noopener noreferrer"
            className="group relative flex items-center gap-3 bg-[#5865F2]/10 border-2 border-[#5865F2]/30 px-8 md:px-12 py-4 rounded-2xl hover:border-[#5865F2] hover:shadow-[0_0_40px_rgba(88,101,242,0.4)] transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] overflow-hidden"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-[#5865F2]/0 to-[#5865F2]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <svg
              ...
              viewBox="0 0 24 24"
              fill="currentColor"
              className="text-[#5865F2] group-hover:text-white transition-colors relative z-10"
            >
            ...
            <span className="text-[12px] font-black uppercase tracking-[0.3em] text-[#5865F2] group-hover:text-white transition-colors italic relative z-10">
               Únete a WarmiFlow
            </span>
            <div className="absolute -inset-full bg-gradient-to-r from-transparent via-white/5 to-transparent skew-x-[-25deg] group-hover:animate-shine" />
          </a>
